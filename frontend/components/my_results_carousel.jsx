...
class MyResultsCarousel extends React.Component {
    ...
    setTimeout(() => {
      if (direction === 1) {
        $(carousel)
          ...
          .before($(carousel).children(".slide:last"));
      } else if (direction === -1) {
        $(carousel)
          .children(".slide:last")
          .after($(carousel).children(".slide:first"));
      }
  ...
  closeExpandingBlock() {
    ...
    const currentSlide = $(expandingBlock)
      .parent(".carousel-component")
      .children(".wrap")
      .children(".window")
      .children(`#carousel_results`)
      .children(".slide");
    $(".slide").css("border-width", "0");
  ...
  playMovie() {
    ...
    const expandingBlock = document.getElementById(`expanding-block-results`);
    const movieTitle = $(expandingBlock)
      .find("h2")
      .text();
    ...
    return ownProps.history.push(`/play/${movieId}`);
  }
  ...
  toggleAddMovieToList() {
    ...
    if (currentButtonText.includes("+")) {
      ...
      deleteListMovie(currentUser.id, movie.id).then(response => {
        $(".added-or-not").html(
      ...
      <div className="carousel-component">
        ...
        <div className="wrap">
          <div className="window-results">
            ...
            <span id={"next_results"} onClick={() => this.shiftSlide(-1)}>
              <FaAngleRight className="scroll-arrow" />
            </span>
          </div>
        </div>
        <div
          className="hide-element expanding-block"
          id={"expanding-block-results"}
        >
          <FaClose
            onClick={() => this.closeExpandingBlock()}
            className="window-close-x"
          />
          <FaPlayCircleO
            onClick={() => this.playMovie()}
            className="expanding-block-play-btn"
          />
          <div className="expanding-block-left">
            <div
              className="added-or-not"
              onClick={() => this.toggleAddMovieToList()}
            />
          </div>
        </div>
      </div>
